...
//盘亏单管理
class InventoryLossController extends AdminBaseController
{
    ...
    /**
     * @api {POST} /admin/InventoryLoss/get_inventory_loss_list 盘亏单列表
     * @apiGroup 盘亏单管理
     * @apiName get_inventory_loss_list
     * @apiVersion 1.0.0
     * @apiDescription 盘亏单列表
     * @apiParam (请求参数) {int} shop_id 门店（仓库）ID 可空
     * @apiParam (请求参数) {str} item_name 商品名称 可空
     * @apiParam (请求参数) {str} sn 盘亏单号 可空
     * @apiParam (请求参数) {str} creator_name 入库制单 可空
     * @apiParam (请求参数) {str} time 盘亏单时间段 可空
     ...
     * @apiSuccess (返回参数) {array} data.list 盘亏单列表数据集
     ...
     * @apiSuccess (返回参数) {str} data.list.sn 盘亏单编号
     * @apiSuccess (返回参数) {int} data.list.creator_id 不用管
     * @apiSuccess (返回参数) {str} data.list.creator_name 制单人
     * @apiSuccess (返回参数) {int} data.list.status 盘亏单状态 0未入库 1已入库,
     * @apiSuccess (返回参数) {str} data.list.time 盘亏单时间
     ...
     * @apiSampleRequest /admin/InventoryLoss/get_inventory_loss_list
     */
    public function get_inventory_loss_list()
    {
...
//        ];
    ...
    /**
     * @api {POST} /admin/InventoryLoss/get_one_inventory_loss_info 查看盘亏单详细信息
     * @apiGroup 盘亏单管理
     * @apiName get_one_inventory_loss_info
     * @apiVersion 1.0.0
     * @apiDescription 查看盘亏单详细信息
     * @apiParam (请求参数) {int} id stock表的ID，不可空
     * @apiSuccess (返回参数) {Int} code 状态码
     * @apiSuccess (返回参数) {str} msg 请求结果 success成功 fail失败
     * @apiSuccess (返回参数) {array} data 返回结果集
     * @apiSuccess (返回参数) {array} data.stock_info 盘亏单数据
     ...
     * @apiSuccess (返回参数) {int} data.stock_info.shop_name 所属仓库
     * @apiSuccess (返回参数) {int} data.stock_info.time 盘亏单日期
     * @apiSuccess (返回参数) {int} data.stock_info.sn 盘亏单编号
     * @apiSuccess (返回参数) {int} data.stock_info.creator_name 盘点制单人
     * @apiSuccess (返回参数) {int} data.stock_info.complete_name 出库人
     ...
     * @apiSuccessExample {json} 返回样例:
     * {"code":200,"msg":"success","data":{"list":[{"id":2,"shop_id":22,"sn":"PY201808060002","creator_id":1,"time":"2018-08-06 14:59:14","status":0,"shop_name":"测试门店10","creator_name":"admin"},{"id":1,"shop_id":22,"sn":"PY201808060001","creator_id":1,"time":"2018-08-06 15:00:55","status":0,"shop_name":"测试门店10","creator_name":"admin"}],"totalPage":1,"totalNum":2,"pageSize":10}}
     * @apiSampleRequest /admin/InventoryLoss/get_one_inventory_loss_info
     */
    public function get_one_inventory_loss_info()
    {
        $id = input('post.id');
        $where['type'] = 2;
        $where['id'] = $id;
    ...
    /**
     * @api {POST} /admin/InventoryLoss/stock_out_confirm 确认出库
     * @apiGroup 盘亏单管理
     * @apiName stock_out_confirm
     * @apiVersion 1.0.0
     * @apiDescription 确认出库
     * @apiParam (请求参数) {int} id stock表的ID，不可空
     * @apiSuccess (返回参数) {Int} code 状态码
     ...
     * @apiSampleRequest /admin/InventoryLoss/stock_out_confirm
     */
    public function stock_out_confirm()
    {
        $id = input('post.id');
        if(!$id){
        ...
        //查询商品当前的库存，当前库存-盘亏库存=实际库存，实际库存写入purchase_price表
        ...
        try{
            ...
            if ($result) {
                ...
                //出库成功之后添加库存预警信息
                foreach ($insert_data as $vv){
                    (new StockModel())->updateStockAlert($vv['item_id'],$vv['shop_id'],$vv['stock']);
                }
                //更新shop_item表中商品库存和冻结库存
                foreach ($stock_item_info as $kk=>$vv){
                    $info = $db->name('shop_item')->where(['shop_id'=>$shop_id,'item_id'=>$vv['item_id']])->find();
                    if (!($db->name('shop_item')->where(['shop_id'=>$shop_id,'item_id'=>$vv['item_id']])->update(['stock_ice'=>$info['stock_ice']-abs($vv['num']), 'stock'=>$info['stock']-abs($vv['num'])])))
                    {
                        $fail+=1;
                    }
                }
