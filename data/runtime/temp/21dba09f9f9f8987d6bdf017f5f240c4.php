<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:54:"themes/admin_simpleboot3/admin\ticket\server_list.html";i:1554867203;s:43:"themes/admin_simpleboot3/public\header.html";i:1554867210;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="utf-8">
...
<div class="wrap js-check-wrap">
    <ul class="nav nav-tabs">
        <li class="active">
            <a href="<?php echo url('ticket/serviceList'); ?>">服务券列表</a>
        </li>
        <li>
            <a href="javascript:parent.openIframeLayer('<?php echo url('Ticket/ticketAddOrUpdate',['type'=>2]); ?>','添加服务券',{});">新增服务券</a>
        </li>
    </ul>
    ...
    <table class="table table-hover table-bordered">
        ...
        <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): if( count($list)==0 ) : echo "" ;else: foreach($list as $key=>$v): ?>
            <tr style="height: 55px;">
                ...
                <td><?php echo (isset($v['service_id']['sname']) && ($v['service_id']['sname'] !== '')?$v['service_id']['sname']:''); ?></td>
                <td><?php echo (isset($v['restrict_num']) && ($v['restrict_num'] !== '')?$v['restrict_num']:''); ?></td>
                <td><?php echo (isset($v['circulation']) && ($v['circulation'] !== '')?$v['circulation']:''); ?></td>
                <td><?php echo (isset($v['expire_date']) && ($v['expire_date'] !== '')?$v['expire_date']:''); ?></td>
                <td><?php echo (isset($v['integral_price']) && ($v['integral_price'] !== '')?$v['integral_price']:''); ?> <?php echo $v['get_way']['status']==1?'元':'积分'; ?></td>
                ...
                <td><?php echo (isset($v['get_way']['name']) && ($v['get_way']['name'] !== '')?$v['get_way']['name']:''); ?></td>
                <td><?php echo $v['type']==1?'代金券':'服务券'; ?></td>
                <td><?php echo isset($v['status'])&&$v['status'] == 1?'上架':'下架'; ?></td>
                <td><?php echo (isset($v['addtime']) && ($v['addtime'] !== '')?$v['addtime']:''); ?></td>
                <td>
                    <a href="javascript:parent.openIframeLayer('<?php echo url('Ticket/ticketAddOrUpdate', ['id'=>$v['id']]); ?>','详情',{});">编辑</a>|
                    <a href="javascript:void (0);" onclick="deletes('<?php echo url('ticket/delTicket'); ?>',<?php echo $v['id']; ?>)">删除</a>
                </td>
            </tr>
...
<script type="text/javascript">
    ...
    $(function () {
        $('.view-btn').click(function(){
            ...
            $.post("<?php echo url('Ticket/getBindShopList'); ?>",{'id':id},function(res){
                if (res.code === 200){
