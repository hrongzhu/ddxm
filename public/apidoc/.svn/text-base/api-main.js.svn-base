...
require([
     ...
     * 点击联动
     */
    $(document).on('click','.sidenav .nav-header',function(){
        var group=$(this).attr('data-group');
        if($(this).hasClass('show')){
            $(this).removeClass('show');
            $('.sidenav li[data-group="'+group+'"]').removeClass("show");
        }else{
            $('.sidenav li[data-group="'+group+'"]').addClass("show");
            $('.sidenav li').not('[data-group="'+group+'"]').removeClass('show');
        }
    });
