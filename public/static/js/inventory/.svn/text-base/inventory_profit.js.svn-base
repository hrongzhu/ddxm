...
var getItemList = function (page) {
	...
	$ajax('/inventory_profit/get_inventory_profit_list', data,
		function (rel) {
			console.log(rel);
...
let _fnGetItemDetail = function (id, calc) {
	$ajax('/inventory_profit/get_one_inventory_profit_info', { id: id }, function (rel) {
		console.log(rel);
...
$(document).on('click', '#table_list .t-calc .remove', function () {
	let id = $(this).parent().parent().parent().data('id');
	$ajax('/Stock/delete', {id: id, type: 1}, function (rel) {
		console.log(rel)
		getItemList(1);
...
$(document).on('click', '.profit-loss #put_storage', function () {
	$ajax('/inventory_profit/stock_in_confirm', {id: productList.id}, function (rel) {
		console.log(rel);
